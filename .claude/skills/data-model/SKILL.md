...
| `Working Entity` | Business unit code | **UPDATE (2026-07-21):** `Hexaware` (internal/corporate staff) is a **legitimate** value — it's just absent from the current 50-row sample because those 2 rows were removed 2026-07-17; do NOT treat its absence as "not allowed". Other values seen in the sample: `AMER`(15) `DTNL`(14) `DTIE`(12) `DTDE`(4) `DTUK`(2) `Entity TBD`(2) `DTAU`(1). The Phase-8 roster schema (v2) lists all of these **plus `Hexaware`**, at `warning` severity — a genuinely new entity is surfaced, not blocked |
| `Region` | Geography | **UPDATE (2026-07-21):** `Hexaware` (internal) is a **legitimate** value, absent from the current sample only because those 2 rows were removed 2026-07-17 — do NOT treat its absence as invalid. Sample values: `EMEA`(32) `AMER`(15) `Region TBD`(2) `APAC`(1). Phase-8 roster schema (v2) allows all of these **plus `Hexaware`**, at `warning` severity (a new region is surfaced, not blocked) |
