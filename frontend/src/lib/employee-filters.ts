...
// --------------------------------------------------------------------------
// Region/Market — one hierarchical, multi-select filter (Region > Market),
// using the same HierarchicalMultiSelect the Search page uses. Market nests
// under Region; each is a leaf you can tick, and ticking a whole Region ticks
// all its Markets. Selected values are encoded so the backend can tell the
...
export function buildRegionMarketItems(
  ...
  const marketsByRegion = new Map<string, Set<string>>()
  for (const e of employees) {
    const region = e.region?.trim()
    if (!region) continue
    const set = marketsByRegion.get(region) ?? new Set<string>()
    const market = e.market?.trim()
    if (market) set.add(market)
    marketsByRegion.set(region, set)
  }
  const items: HierarchicalItem[] = []
  for (const region of Array.from(marketsByRegion.keys()).sort()) {
    items.push({ value: region, label: region })
    for (const market of Array.from(marketsByRegion.get(region) ?? []).sort()) {
      items.push({ value: `${region}::${market}`, label: market, parent: region })
    }
  }
  return items
}

/** Split a selected Region/Market value into its filter contribution: a
 * plain top-level value is a Region; a `"<region>::<market>"` value is a
 * Market (the market alone narrows to the right rows — the backend ANDs
 * region and market, so the parent region isn't also needed). Mirrors the
 * Search page's `splitRegionMarketValue`. */
export function splitRegionMarketValue(value: string): {
  region?: string
  market?: string
} {
...
/** Collapse the hierarchical Region/Market selection into `region[]` /
 * `market[]` server-filter params (undefined when none selected). */
export function regionMarketServerFilters(selected: string[]): {
  region?: string[]
  market?: string[]
} {
